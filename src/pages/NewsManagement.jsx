...
import { Card, Table, Tag, Button, Modal, Form, Input, Select, message, Image } from 'antd'
...
export default function NewsManagement() {
  ...
  const handlePublish = () => {
    publishForm.validateFields().then(v => {
      setNews([{ id: news.length + 1, ...v, publisherName: user?.name || '未知', publisherType: role, status: 'pending', publishDate: new Date().toISOString().split('T')[0], images: [], reviewComment: '' }, ...news])
  ...
  return (
    <Card title="新闻管理">
      ...
      <Modal title="发布新闻" open={publishOpen} onOk={handlePublish} onCancel={() => { setPublishOpen(false); publishForm.resetFields() }} width={600}>
